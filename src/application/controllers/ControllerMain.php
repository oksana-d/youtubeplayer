...
class ControllerMain extends Controller
{
    ...
    public function likeAction(){
        if($_POST) {
            ...
            $idVideo = $this->model->getVideo($_POST['id']);
            if($idVideo == NULL){// видео нет в бд
                $this->model->saveVideo($_POST);
            }
            if($this->model->getLike($idUser['idUser'], $_POST['id']) == NULL){// лайка нет в бд
                $this->model->putLike($idUser['idUser'], $_POST['id']);
            } else $this->model->removeLike($idUser['idUser'], $_POST['id']);
    ...
    public function getDataVideo(array $videos)
    {
        ...
        array_walk($videos, function ($value) use (&$dataset){
            $dataset[] = [
                ...
                'title' => self::removeEmoji($value->toSimpleObject()->snippet->title),
    ...
    function getLike($videos){
        ...
        foreach ($videos as $video){
            $idLike = $this->model->getLike($idUser['idUser'], $video['idVideo']);
            if($idLike == NULL){
               $mas = array_merge($video, array('like' => false));
               array_push($like, $mas);
            } else {
                $mas = array_merge($video, ['like' => true]);
                array_push($like, $mas);
            }
        }
        return $like;
